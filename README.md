...
over everything; tap it and a **post-it** appears on top of your notebook or document.
...
- **Lasso → Add to sticky**: OCR handwriting straight into a new post-it
...
- Live preview: open post-its stay on top of the Manager, so font/size changes show in real time
- **Export** to `.txt`, **backup/restore** to `.json` (import merges, never overwrites)
- Notes stored **privately** (not cloud-synced)

> Text only for now — handwriting inside a post-it isn't supported yet. Tested on Manta (A5 X2) and A5 X.

## Demos

**Lasso handwriting → sticky (OCR)**

![Lasso to sticky](docs/lasso-demo.gif)

**Sticky → note → export**

![Sticky to note and export](docs/sticky-to-note-export-demo.gif)
